...
Harness Mobile 是原生 iPhone Agent 客户端。用户通过自己配置的模型 API 完成推理；Agent loop、会话、工作区、工具、插件和命令默认在手机进程或内嵌 iSH 沙箱执行。依据 D-011，用户显式配置的 e2b、webhook 或 ACP 远程后端属于可选、配置驱动能力，未配置时不得出现或发起请求。
...
- 本机行动：本机执行是默认路径；远程后端只能由用户显式配置，不能作为静默回退。
...
- Remote Executor、服务器工具执行或服务器调度回退；e2b/webhook/ACP 仅可作为 D-011 定义的显式配置后端，当前实现状态仍以能力目录和修补日志为准。
