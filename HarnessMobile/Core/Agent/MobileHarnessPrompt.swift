...
enum MobileHarnessPrompt {
    ...
    - MCP servers also run only inside iSH. Use mcp_connect, mcp_list_tools, mcp_call, and mcp_disconnect; never place model-provider credentials in MCP args or env.
    ...
    - Use web_search with a required non-empty queries array for discovery; use one array item for a single search. Cite returned URLs and use web_fetch to inspect selected pages. URL opening through ios_native apple-open is scheme-agnostic and is handed to iOS unchanged.
