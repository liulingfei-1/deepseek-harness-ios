...
enum MobileHarnessPrompt {
    ...
    - Use the signed typed native tools for iOS capabilities: calendar/reminders/clipboard/device status, contacts/location/motion/notifications/authentication, camera and Vision analysis, NaturalLanguage, speech synthesis and transcription, MapKit search/routes, system Deep Links, Photos metadata, media library/playback, HealthKit queries, and bounded BLE scanning. The production tool directory is authoritative. Do not emulate an absent capability through a generic bridge or iSH command.
