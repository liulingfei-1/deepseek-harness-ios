...
- `AppModel.swift` is an `@MainActor` composition root, not a home for new transport, storage, compiler, or plugin-runtime algorithms. It is about 8,600 lines: locate a symbol with `rg -n`, then read only the nearby window.
