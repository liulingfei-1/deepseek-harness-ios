...
struct TerminalCellMetrics: @unchecked Sendable {
...
enum TerminalRedrawLog {
    ...
    nonisolated(unsafe) static var pendingInputAt: Double? = nil
    nonisolated(unsafe) static var pendingInputChar: String = ""
}
...
final class TerminalScrollContainerView: UIView {
    ...
    /// Characters that can legally appear inside a URL. Used to
    ...
    /// Scan `str` for URLs and add `.link` attributes in-place.
...
extension TerminalScrollContainerView: UITextViewDelegate {
    func textView(_ textView: UITextView,
        ...
        // Do not impose an app-level scheme policy. Every detected link is
        // handed to UIKit, which makes the final decision for the target app
        // and for unsupported schemes.
