...
- 真机构建与安装：2026-08-31 使用自动签名 Team `2SH3TUR2JL` 构建 `/tmp/hm-device-ui-0831/Build/Products/Debug-iphoneos/HarnessMobile.app`，并由 `devicectl` 将 `com.llf.harnessmobile` 覆盖安装、启动到配对的 iPhone 16 Pro。2026-09-01 最新 UI 包 `/tmp/hm-device-ui-0901/Build/Products/Debug-iphoneos/HarnessMobile.app` 也已签名构建成功，但设备处于 `unavailable`，覆盖安装返回 `CoreDeviceError 4016`，待手机解锁并恢复可信连接后重试。旧包已安装不代表最新包已安装；逐页触控、真实权限、插件 Host 和 VoiceOver 仍为 `VERIFY`。
