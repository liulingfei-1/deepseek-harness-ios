...
- 调整前截图：`/tmp/native-plugin-settings-before-attachments-0902/10FFA333-DD66-4999-83F2-6F7BC62C56BA.png`。导航标题已显示插件名称，“运行方式”仍重复“插件 / File Memory Native”，占用首屏并把真正可编辑的配置后移。
- 调整：删除重复插件名称行；生效方式、存储位置、schema 驱动配置、草稿、保存、放弃和恢复默认值逻辑不变。
...
- **更正（第三轮复查）**：上述"底部搜索框"实为 iOS 26 Liquid Glass 的系统搜索栏位置——`searchable` 默认 `.automatic` placement 在 iOS 26+ 渲染为底部玻璃搜索（首页、插件市场如此；模型选择、轨迹页显式用 `.navigationBarDrawer(displayMode: .always)` 故保持在顶部）。内容从玻璃下穿过属系统样式，非遮挡缺陷；滚动到底后末行可见性待真机滚动复核。两处 placement 差异（顶部/底部）为现有代码事实，是否统一交产品决策。
- 其余复查页面（手机权限、模型行为、Agent Bundle、插件详情、GitHub 安装、删除/导出确认、重命名）视觉质量合格，未做无意义改动。

## 全量重截图复核轮（2026-09-02 傍晚）

- 按"全部重新截图再过一遍"要求：10 个专项测试类在最新代码上重跑（`/tmp/hm-refresh-all.xcresult`），导出 40 张命名截图（`/tmp/hm-refresh-named/`，标准字号深色）。
- 全部既有修复经新截图逐一验证生效：计划审阅紧凑列表、工具页 iSH 图标（`.primary` tint）、轨迹统计一行放下、后台任务 `ContentUnavailableView` 空态、Agent 预设 `.large` detent 全 4 预设可见、模型选择滚动修复。
