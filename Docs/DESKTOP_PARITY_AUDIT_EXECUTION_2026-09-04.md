...
| 005 | VERIFY | ACP NDJSON iSH transport、可配置 provider catalog 与 `subagent`/Jobs 路径已接线；`runAndWait` 取消会向活动 session 发送 `session/cancel` | 真实 ACP entrypoint、持久化 provider 选择、EOF/exit/reconnect |
...
| 010 | VERIFY | loopback `/health`、`/status`、`/sessions`、`/api/schema` 与 `/api/session` 已启动并动态投影；真实 `URLSession` GET/POST 已验证，且已修复分片 HTTP 请求聚合 | RPC 写入、端口冲突、生命周期、真机 |
