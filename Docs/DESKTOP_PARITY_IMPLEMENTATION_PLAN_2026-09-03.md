...
| PARITY-013 webhook | VERIFY | GitHub delivery/event/payload envelope、loopback POST `/webhook/github`、有界去重及跨重启 delivery state 已实现；支持显式 secret 的 HMAC-SHA256 校验；修复 AppModel sink 初始化时提前 claim delivery 的 bug，Job 投影路径已接通；10 个 LocalStateServer 测试通过；rule/重试/Agent 唤醒仍待实现 |
