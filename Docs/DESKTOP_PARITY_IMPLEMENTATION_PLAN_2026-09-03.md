...
| 当前移动端锁定上游 | `d347e703908d0406b7a7ef80e3a0e594d86b2215` / GitHub master（`v0.1.3-alpha.1`，2026-09-04） |
...
| SwiftPM 基线（2026-09-05复核） | 960 tests，5 skipped，0 failures |
...
- **步骤**：建立 provider plugin protocol；迁移静态 provider；实现 capability/cache；补 OAuth record/refresh（record、single-flight、RFC 6749 自动刷新与单次 401 token-rotation retry 已完成，待 provider-specific flow）；统一 request/replay metadata；逐 provider fixture 对比上游。
