...
| 42 | BROWSER-001 | 隔离 WKWebView Browser service/provider/tools | LOG-001,PLUGIN-001 | `TODO` | `BLOCKED` | MINIS-10 |
| 43 | SYNC-001 | immutable event suffix 同步 envelope/tombstone | STORE-003,ATTACH-001 | `TODO` | `BLOCKED` | MINIS-08 |
| 44 | OBS-001 | runtime invariant registry + 默认无正文诊断 | LOG-001,PLUGIN-001 | `TODO` | `BLOCKED` | 上游 invariants/telemetry + MINIS-19 |
| 45 | OBS-002 | MetricKit、hang/watchdog、opt-in perf/resource trace | OBS-001,BG-010 | `TODO` | `BLOCKED` | MINIS-19 |
| 46 | RELEASE-001 | Capability Manifest 与生产目录/Xcode 测试覆盖 | AGENT-002,PLUGIN-001 | `TODO` | `BLOCKED` | 当前 SwiftPM exclude 缺口 |
...
| 2026-08-26 | BROWSER-001 审计 | 复核锁定的 OpenMinis `Agent/BrowserUse/BrowserUseManager.swift`、`BrowserTabPool`、`BrowserUseJavaScript`、Cookie/Download/ResourceMonitor 链路；该实现依赖完整 WKWebView service、每 tab 生命周期、DOM/截图/下载事件和多种 cookie/JS 操作。当前 Harness 只有手机直连 `web_fetch`/`web_search`，没有 WebKit service/provider/tool target；源码存在不等于移动端能力已接入。若继续实现，必须先建立 per-session 最多 3 tab、进程 cap、non-persistent data store、禁止任意 JS/cookie、WebContent termination 和模型可见 provenance 的原生 seam，不能把上游调试接口直接暴露给 Agent。 | 现有 `WebFetchToolTests` 和无远程执行审计只证明 fetch/search 手机直连边界；没有 Browser service/provider/tools、A/B tab isolation、eviction、termination 或 secret canary 证据。 | `BROWSER-001 TODO/BLOCKED`：保持未实现，下一阶段需先完成上述安全 seam 和独立 fixtures 后再进入 Xcode/真机验收；不能以 OpenMinis 源码或 `web_fetch` 代替。 |
