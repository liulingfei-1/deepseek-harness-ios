...
| 2026-08-27 | BROWSER-001 termination/provenance | 在受限 Browser seam 上补 `webContentTerminated` typed outcome：WebKit `webViewWebContentProcessDidTerminate` 释放 renderer，Service 同步移除对应 tab，后续访问变为 `tabNotFound`；所有结果统一带 `on_device_browser`、kind、pageURL 和 UTF-8 byteCount provenance，正文/凭据不复制进 provenance，继续由现有 `toolResult` durable event 记录模型可见输出。 | `HarnessBrowserServiceTests` 7/7（A/B 隔离、LRU/资源 discard、输出上限、termination 清理、provenance、schema 白名单）；SwiftPM 聚焦 7/7。Xcode Beta arm64 generic Simulator 多次尝试仍触发 AppModel SwiftDriver “exit 0 but produced no further output”，未计为通过；Plugin Host、无远程执行审计、upstream parity、`git diff --check` 待本轮收尾复跑。 | `BROWSER-001 PARTIAL/BLOCKED`：termination/provenance 自动门已补，仍缺真实 iOS backend/真机、下载记录与 secret-canary fixture，不能写成完整 BrowserUse 或 DONE；下一步实现有界 download seam 或明确拒绝路径，再做 Xcode/真机验收。 |
