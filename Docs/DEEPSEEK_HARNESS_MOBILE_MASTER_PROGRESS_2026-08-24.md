...
| 系统入口 | 应写 Agent inbox/session，而非临时 UI 草稿 | App Intents 已通过 durable inbox 提交 List/Status/Open/Retry/SendPrompt；Share 已通过受限 App Group handoff 接纳 WorkspaceStore；Widget 仍未实现 | `VERIFY/PARTIAL`：Intent/Share 自动与模拟器门已闭合；Shortcut、冷启、审批、Share Extension force-close/真实 provider 和 iPhone 16 Pro 验收仍待执行 | `INTENT-001`、`SHARE-001`、`WIDGET-001` |
...
| 41 | WIDGET-001 | 只读 session/run projection Widget | INTENT-001 | `TODO` | `READY` | Minis 系统闭环，原生 projection |
