...
| 45 | OBS-002 | MetricKit、hang/watchdog、opt-in perf/resource trace | OBS-001,BG-010 | `VERIFY` | `CLOSED` | 有界数值遥测、launch marker、前台 heartbeat watchdog、按 identity 去重的后台超时、MetricKit aggregate 与显式会话工作区导出已通过自动/模拟器门；MetricKit 延迟投递、真实后台 expiration、长时运行和 iPhone 16 Pro 仍待验收 |
...
| 47 | RELEASE-002 | 全门禁、真机报告、上游升级差异门 | BG-012,RELEASE-001,OBS-002 | `VERIFY` | `BLOCKED` | 自动测试/Node/边界/upstream lock/parity/manifest/upgrade-check/diff 门已通过；iPhone 16 Pro 已完成签名构建、安装和启动首页复验；真实 provider/API、iSH、插件、后台/MetricKit、长会话和会话工作区导出仍待执行，详见 `Docs/RELEASE_VERIFICATION_2026-08-28.md` |
...
| 2026-08-28 | OBS-002 runtime observability | 新增 `RuntimeTelemetryStore`：128 条/64 KiB 有界数值记录，默认关闭的热状态、低电量与前后台采样，最小 launch marker，前台 heartbeat watchdog，以及按 session/run/generation 且按来源去重的 background timeout。MetricKit 仅接收 exit/hang aggregate，绝不保留 raw payload、JSON、call stack、prompt、工具输入输出、URL/header/cookie、API key 或环境变量。诊断导出仅在用户主动点击时写入当前会话 SHA-256 哈希隔离的 `Downloads/session-<digest>/Harness-Diagnostics-YYYYMMDD-HHMMSS.log`；固定文件名、2 MiB 上限和 workspace containment 均 fail-closed。 | 聚焦 `RuntimeTelemetryTests|BackgroundPreferencesTests|HarnessTraceStoreTests` 20/20；SwiftPM 全量 811 tests、3 skipped、0 failures；`AgentRuntimeTests.testPinnedDeepSeekToolSchedulerDifferentialFixture` 单独复跑 1/1 通过；Xcode Beta arm64 generic iOS Simulator `/tmp/hm-obs002-xcode` build 退出码 0；Plugin Host、无远程执行审计、upstream parity、capability manifest、upgrade-check 与 `git diff --check` 全部 PASS。 | `OBS-002 VERIFY/CLOSED`：MetricKit 为系统延迟投递；真实 iPhone 16 Pro 的 MetricKit 收集、前后台切换/timeout、锁屏与长时运行、以及真实会话导出仍未执行，不能写成 `DONE`；下一项 `RELEASE-002`。 |
| 2026-08-28 | OBS-002 watchdog crash follow-up | 真机启动发现 `EXC_BREAKPOINT/SIGTRAP`，稳定触发于 `RuntimeHangWatchdog.start()` 的 DispatchSource handler；`251b70e` 将 handler 显式设为非 MainActor `@Sendable` 并在私有队列中 detached 调度 actor 访问。 | 修复后 `RuntimeTelemetryTests` 8/8；SwiftPM 全量 812 tests、3 skipped、0 failures；Xcode Beta arm64 iOS Simulator 与 iPhone 16 Pro arm64 build 均 `BUILD SUCCEEDED`；真实设备重新安装、启动后进程保持运行，首页截图已归档到 `Docs/Evidence/release002-iphone16pro-launch-2026-08-28.png`。 | `OBS-002 VERIFY/CLOSED`：真机 MetricKit、后台/长会话和会话工作区导出仍待验收；详见 `Docs/RELEASE_VERIFICATION_2026-08-28.md`。 |
| 2026-08-28 | RELEASE-002 发布门与真机启动复验 | 收集并固定自动门、上游 lock、Xcode generic Simulator/真实设备构建、安装启动及版本化报告；修复启动时 watchdog executor trap 后重新签名安装。 | `Scripts/upgrade-check.sh` PASS；SwiftPM 全量 812 tests、3 skipped、0 failures；Plugin Host、无远程执行审计、upstream parity、capability manifest、`git diff --check` PASS；iPhone 16 Pro 主 App 进程保持运行并归档首页截图。 | `RELEASE-002 VERIFY/BLOCKED`：真实 API/provider、图片、iSH、插件、后台/MetricKit、锁屏/低电量/热压力、长会话和真实会话工作区导出仍待验收；不能写成发布完成。 |
| 2026-08-28 | RELEASE-002 设备 UI 回归尝试 | 现有 `HarnessMobileOnboardingUITests/testResetAlwaysReturnsToOnboarding` 已用同一 development team 成功签名并部署到 iPhone 16 Pro；XCTest runner 尚未建立通道即被 Xcode Beta 的诊断观察器终止。 | 首次失败为 test target 缺少 development team；补签后结果包 `/tmp/hm-release002-ui-device-final/Logs/Test/Test-HarnessMobile-2026.08.28_10-30-57-+0800.xcresult` 报 `xcrun: error: unable to find utility "devicectl"`，runner code 74；主 App 仍在运行且无新增 HarnessMobile crash log。 | `RELEASE-002 VERIFY/BLOCKED`：设备 UI 测试未执行到断言，保留为环境阻塞；不得写成 UI 真机通过。 |
| 2026-08-28 | RELEASE-002 设备 UI 工具链复核 | `xcode-select -p` 为 `/Library/Developer/CommandLineTools`，Beta 自带 `devicectl` 可独立运行；在保留 `rg` 的 PATH 下增加临时 `/tmp/hm-xcrun-shim/xcrun` 仍无法改变 Xcode 观察器的默认 `xcrun` 查找。 | 最新结果包 `/tmp/hm-release002-ui-device-shim/Logs/Test/Test-HarnessMobile-2026.08.28_10-36-35-+0800.xcresult` 仍为 runner code 74、未进入断言；`/usr/bin/devicectl` 可读取设备，主 App 进程保持运行。 | `RELEASE-002 VERIFY/BLOCKED`：同一宿主工具链阻塞已复现，不能写成设备 UI 测试通过。 |
