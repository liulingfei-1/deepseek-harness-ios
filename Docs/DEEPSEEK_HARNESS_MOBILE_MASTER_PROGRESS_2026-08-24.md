...
| 2026-08-26 | BROWSER-001 seam | 在复核上游 OpenMinis BrowserUse 链路后，新增 `HarnessBrowserService` actor、`HarnessBrowserTool` 和 iOS-only `HarnessBrowserWebKitBackend`：每 session 最多 3 tab、全局 6 tab LRU、A/B 隔离、HTTP/HTTPS 无凭据 URL 校验、non-persistent `WKWebView`、固定 `document.body.innerText`/截图动作；工具 schema 不暴露 cookie、headers、任意 JavaScript 或远程执行。 | `HarnessBrowserServiceTests` 5/5；Mac SwiftPM Browser 子集通过；iOS backend 与 App/Xcode/真机 WebContent termination、DOM/截图 provenance、download、secret canary 尚未完成验证。 | `BROWSER-001 PARTIAL/BLOCKED`：安全 seam 与工具入口已落盘，但仍不能称完整 BrowserUse；下一步补 WebContent termination/下载与 provenance fixtures，再做 Xcode/真机验收。 |
