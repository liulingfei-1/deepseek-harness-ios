...
| PARITY-015 | Session controller `page`：按消息边界向前分页并返回 raw/chunk records | `AppModel.handleLocalStateRPC` → `SessionTrajectoryRepository.allEvents` → `localSessionPagePayload` | VERIFY | `session/page` 已接入并有真实 JSONL 回归；ChunkRow packing、真实 client/真机仍待验证 |
