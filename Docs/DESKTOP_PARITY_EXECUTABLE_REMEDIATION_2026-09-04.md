...
| PARITY-017 | Session controller `modelCatalog`：默认路由、provider groups 与 reasoning metadata | `AppModel.handleLocalStateRPC` → `ProviderProfileDirectory` → `localSessionModelCatalogPayload` | VERIFY | 已接入 profile-backed catalog RPC、`refresh/profileId` 强制重拉、失败诊断和 generation bump；真实 provider/OAuth/真机仍待验证 |
