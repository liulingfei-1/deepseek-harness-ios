...
| PARITY-017 | Session controller `modelCatalog`/`selectModel`：默认路由、provider groups、reasoning metadata 与会话选择 | `AppModel.handleLocalStateRPC` → `ProviderProfileDirectory`/`setSessionModelConfiguration` | VERIFY | 已接入 profile-backed catalog RPC、`refresh/profileId` 强制重拉、失败诊断、generation bump 和 `session/selectModel` 持久化选择；真实 provider/OAuth/真机仍待验证 |
| PARITY-022 | Session-addressed `skills/list` | `AppModel.handleLocalStateRPC` → `MobileSkillRegistry` | VERIFY | 已接入独立 `skills` controller schema 与 user-invocable 元数据投影；真实 Desktop client/Session preset scope 仍待验证 |
