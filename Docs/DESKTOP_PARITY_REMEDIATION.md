...
| VERIFY-007 | VERIFY | iPhone 16 Pro 已完成真实签名构建、安装和启动首页复验；真实 API 连通、图片、iSH、插件与后台验收仍待执行 |
