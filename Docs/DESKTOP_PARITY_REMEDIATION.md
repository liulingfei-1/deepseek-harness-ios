...
| 2026-08-26 | BROWSER-001 审计 | 复核上游 `Agent/BrowserUse/BrowserUseManager.swift`、`BrowserTabPool`、`BrowserUseJavaScript` 及 Cookie/Download/ResourceMonitor 链路；当前 Harness 仅有手机直连 `web_fetch`/`web_search`，未接入 WKWebView browser service/provider/tools。直接复制上游会同时引入任意 JS、cookie 和共享 WebContent 状态，违反本项目的本机工具边界与模型可见 provenance 要求。 | 当前只有 `WebFetchToolTests` 与无远程执行审计；缺少 per-session 3-tab/process cap、A/B isolation、serial action、eviction、WebContent termination 和 secret canary fixtures。 | `BROWSER-001 TODO/BLOCKED`；下一阶段必须先设计并测试 non-persistent per-session Browser seam、受限动作 allowlist、DOM/截图/下载 provenance 与资源终止闭合，再进行实现/真机验收；不能以源码存在或 `web_fetch` 代替。 |
