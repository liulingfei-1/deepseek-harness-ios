...
| Native mobile capability providers | Partial | P2 | Research-defined mobile extension | Signed typed Swift providers now cover contacts, calendar/reminders, clipboard/device state, location/motion/notifications/authentication, Vision, NaturalLanguage, foreground speech transcription, speech synthesis, MapKit search/routes, system Deep Links, PhotoKit metadata, media search/playback, HealthKit reads, and bounded BLE scan. The generic OpenMinis bridge remains removed | Validate permission UI and real results on iPhone; add PhotoKit mutation/export and protocol-specific BLE connect/read/write. HomeKit and NFC remain blocked by the current signing capabilities rather than silently routed through iSH | `HarnessMobile/Core/Tools/IOSCapabilityTools.swift`; `HarnessMobile/Core/Tools/MobileNativeTools.swift`; `HarnessMobile/Core/Tools/DeviceCapabilitiesTool.swift` |
