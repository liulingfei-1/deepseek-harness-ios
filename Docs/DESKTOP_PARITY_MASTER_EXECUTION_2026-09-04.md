...
- [x] session/workspace follow 的 SSE/chunked carrier 与 `URLSession` stream client；session 首帧 snapshot、后续事件 cursor 已有 live loopback 回归。
- [ ] 原生事件订阅（当前为 250ms persistence polling bridge）、workspace baseline/increment、断线重连 generation、WebSocket、端口冲突、前后台启停和 iPhone 16 Pro 证据。
